...
void BaseLabRunAction::ConfigureCommands()
{
  fMessenger = std::make_unique<G4GenericMessenger>(this, "/baseLab/output/", "Output metadata control");

  auto& tagCmd = fMessenger->DeclareMethod("tag", &BaseLabRunAction::SetOutputTag,
  ...
  auto& depthCmd = fMessenger->DeclareMethodWithUnit(
    "depth", "cm", &BaseLabRunAction::SetOutputDepth, "Set depth metadata stored in runinfo ntuple.");
...
void BaseLabRunAction::SetOutputTag(const G4String& tag)
{
  if (tag.empty()) {
    G4ExceptionDescription msg;
    msg << "Invalid output tag '" << tag << "'. Tag must not be empty.";
    G4Exception("BaseLabRunAction::SetOutputTag", "BaseLabOutput001", FatalException, msg);
    return;
  }
  ...
  for (auto ch : tag) {
    ...
    if (!std::isalnum(uch) && ch != '-' && ch != '_') {
      ...
      G4Exception("BaseLabRunAction::SetOutputTag", "BaseLabOutput002", FatalException, msg);
      return;
    }
  }
...
G4String BaseLabRunAction::SanitizeForFileName(const G4String& value)
{
  G4String out;
...
G4String BaseLabRunAction::BuildOutputFileName() const
{
  const auto tag = SanitizeForFileName(fOutputTag);
  if (tag.empty()) {
    return "baseLab-default.root";
  }
  return "baseLab-" + tag + ".root";
}
...
void BaseLabRunAction::BeginOfRunAction(const G4Run*)
{
  ...
  if (fEnableRadioactiveDecay
    ...
    if (status != 0) {
      ...
      G4Exception("BaseLabRunAction::BeginOfRunAction", "BaseLabDecay001", FatalException, msg);
      return;
    }
  }
...
void BaseLabRunAction::AddEventScoring(G4double eventDose, G4double eventEnergyDeposit, G4double eventTrackLength)
{
  fRunDose += eventDose;
...
G4double BaseLabRunAction::GetRunDose() const
{
...
G4double BaseLabRunAction::GetRunTrackLength() const
{
